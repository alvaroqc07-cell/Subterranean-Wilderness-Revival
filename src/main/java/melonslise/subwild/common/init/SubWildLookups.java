...
public final class SubWildLookups
{
	...
	public static HashMap<Block, HashMap<Block, Block>> ORE_TABLE = new HashMap<>(13 + 4);
	...
	public static void init()
	{
		...
		FROZEN_SPELEOS = new HashMap<>(9 + 1 * 4);
	...
	// 1.20.1: Tag.Named reemplazado por TagKey — iteramos con ForgeRegistries
	public static <K, V> void put(Map<K, V> lookup, TagKey<K> tag, V value)
	{
		ForgeRegistries.BLOCKS.tags().getTag((TagKey<Block>) tag).stream()
			.forEach(block -> lookup.put((K) block, value));
	}
}
