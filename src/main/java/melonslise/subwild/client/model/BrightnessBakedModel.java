...
@OnlyIn(Dist.CLIENT)
public class BrightnessBakedModel extends BakedModelWrapper<BakedModel>
{
	...
	{
		List<BakedQuad> quads = this.originalModel.getQuads(state, side, rand, data, renderType);
		for(BakedQuad quad : quads)
		{
			if(quad.getVertices()[6] == 0x00F00F0)
				break;
			// 1.20.1: TextureAtlasSprite.getName() → sprite.contents().name()
			if(this.filter.test(quad.getSprite().contents().name()))
				transformQuad(quad);
		}
		return quads;
	}
